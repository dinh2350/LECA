    ...
    <div ref={containerRef} className="flex flex-col gap-1 w-full">
      ...
      <div ref={boxRef}>
        <button
          ...
          onBlur={props.onBlur}
          className={cn(
            'flex h-9 w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-1 text-sm text-left text-[var(--color-foreground)] shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--color-accent)] disabled:cursor-not-allowed disabled:opacity-50',
          ...
          <Virtuoso
            ...
            itemContent={(_index, item) => (
              <button
                type="button"
                className={cn(
                  'w-full text-left px-3 py-2 text-sm hover:bg-[var(--color-accent)]/10 hover:text-[var(--color-accent)] transition-colors',
                  props.value &&
                    props.keyExtractor(item as T) ===
